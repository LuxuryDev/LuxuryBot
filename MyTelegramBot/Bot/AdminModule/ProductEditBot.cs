...
namespace MyTelegramBot.Bot
{
    ...
    public class ProductEditBot: BotCore
    {
        ...
        private CurrencyListMessage CurrencyListMsg { get; set; }
        /// <summary>
        /// Id товара
        /// </summary>
            ...
            if (IsOwner())
            {
                switch (base.CommandName)
                {
                    ...
                    case "BackToProductEditor":
                        ...
                        ///Пользователь нажал на "Валюта", высылаем сообещние со списком всех валют
                    case ProudctCurrencyCmd:
                        return await SendCurrencyList();
        ...
        /// <summary>
        /// Отправляем список всех валют
        /// </summary>
        /// <returns></returns>
        private async Task<IActionResult> SendCurrencyList()
        {
            try
            {
                if (await EditMessage(CurrencyListMsg.BuildMessage()) != null)
                    return OkResult;
        ...
        public async Task<IActionResult> SendUnitList()
        {
            try
            {
                if (UnitListMsg != null &&await EditMessage(UnitListMsg.BuildMessage()) != null)
                    return OkResult;
        ...
        private async Task<IActionResult> SendProductAdminMsg(int MessageId=0)
        {
            ...
            if (MessageId > 0 && await EditMessage(AdminProductFuncMsg.BuildMessage()) != null)
                return OkResult;
