...
{
    public class AdminHelpDeskMessage : Bot.BotMessage
    {
        ...
        public async Task<AdminHelpDeskMessage> BuldMessage()
        {
            using (MarketBotDbContext db = new MarketBotDbContext())
            {
                if (this.HelpDesk == null)
                    this.HelpDesk = await db.HelpDesk.Where(h => h.Id == HelpDeskId).Include(h => h.HelpDeskAttachment).
